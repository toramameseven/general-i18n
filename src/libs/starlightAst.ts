...
export function getStarlightLocalesConfigFromCode(code: string) {
  ...
  const locales = getStarlightLocales(ast, starlightConfig)
  const rootConfigSetting = getStarlightDefaultLocaleConfig(starlightConfig)
  const isUseRoot = rootConfigSetting === 'root' || rootConfigSetting === undefined
  const defaultLocale =
    isUseRoot ?
      Object.entries(locales).find(([name]) => name === 'root')?.[1].lang : rootConfigSetting
...
function getStarlightLocales(ast: File, starlightConfig: ObjectExpression) {
